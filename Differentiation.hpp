...
template <WhichD W>
constexpr double stencil3_func(double x, double y, double h_x, double h_y)
{
...
template <WhichD W>
constexpr double stencil5_func(double x, double y, double h_x, double h_y)
{
  if constexpr (W == WhichD::x)
  {
    return (F(x - 2 * h_x, y) - 8 * F(x - h_x, y) + 8 * F(x + h_x, y) -
            F(x + 2 * h_x, y)) /
           (12 * h_x);
  }
  else if constexpr (W == WhichD::y)
  {
    return (F(x, y - 2 * h_y) - 8 * F(x, y - h_y) + 8 * F(x, y + h_y) -
            F(x, y + 2 * h_y)) /
           (12 * h_y);
  }
  else if constexpr (W == WhichD::xx)
  {
    return ((-1) * F(x - 2 * h_x, y) + 16 * F(x - h_x, y) - 30 * F(x, y) +
            16 * F(x + h_x, y) - F(x + 2 * h_x, y)) /
           (12 * h_x * h_x);
  }
  else if constexpr (W == WhichD::yy)
  {
    return ((-1) * F(x, y - 2 * h_y) + 16 * F(x, y - h_y) - 30 * F(x, y) +
            16 * F(x, y + h_y) - F(x, y + 2 * h_y)) /
           (12 * h_y * h_y);
  }
  else if constexpr (W == WhichD::xy)
  {
    return ((F(x - 2 * h_x, y - 2 * h_y) - F(x + 2 * h_x, y - 2 * h_y) +
             8 * F(x + h_x, y - 2 * h_y) - 8 * F(x - h_x, y - 2 * h_y)) /
            (12 * h_x)) /
               (12 * h_y) -
           2 *
               ((F(x - 2 * h_x, y - h_y) - F(x + 2 * h_x, y - h_y) +
                 8 * F(x + h_x, y - h_y) - 8 * F(x - h_x, y - h_y)) /
                (12 * h_x)) /
               (3 * h_y) +
           2 *
               ((F(x - 2 * h_x, y + h_y) - F(x + 2 * h_x, y + h_y) +
                ...
                (12 * h_x)) /
               (3 * h_y) -
           ((F(x - 2 * h_x, y + 2 * h_y) - F(x + 2 * h_x, y + 2 * h_y) +
             8 * F(x + h_x, y + 2 * h_y) - 8 * F(x - h_x, y + 2 * h_y)) /
            (12 * h_x)) /
               (12 * h_y);
  }
...
private:
  constexpr AAD22(int i, double v)
      : m_val(v), m_d1{(i == 0) ? 1.0 : 0.0, (i == 0) ? 0.0 : 1.0}, m_d2{0, 0,
                                                                         0}
  {
  }
...
public:
  ...
  constexpr AAD22 &operator+() { return *this; }
  ...
  constexpr friend AAD22 operator*(const AAD22 &lhs, const AAD22 &rhs)
  {
    ...
    res.m_d2[0] = 2 * lhs.m_d1[0] * rhs.m_d1[0] + rhs.m_val * lhs.m_d2[0] +
                  lhs.m_val * rhs.m_d2[0];

    res.m_d2[1] = 2 * lhs.m_d1[1] * rhs.m_d1[1] + rhs.m_val * lhs.m_d2[1] +
                  lhs.m_val * rhs.m_d2[1];
  ...
  constexpr friend AAD22 operator/(const AAD22 &lhs, const AAD22 &rhs)
  {
    ...
    res.m_d2[0] = (-rhs.m_val * (2 * lhs.m_d1[0] * rhs.m_d1[0] +
                                 lhs.m_val * rhs.m_d2[0]) +
                   lhs.m_d2[0] * rhs.m_val * rhs.m_val +
                   2 * lhs.m_val * rhs.m_d1[0] * rhs.m_d1[0]) /
                  (rhs.m_val * rhs.m_val * rhs.m_val);

    res.m_d2[1] = (-rhs.m_val * (2 * lhs.m_d1[1] * rhs.m_d1[1] +
                                 lhs.m_val * rhs.m_d2[1]) +
                   lhs.m_d2[1] * rhs.m_val * rhs.m_val +
                   2 * lhs.m_val * rhs.m_d1[1] * rhs.m_d1[1]) /
                  (rhs.m_val * rhs.m_val * rhs.m_val);

    res.m_d2[2] = (-rhs.m_val * (lhs.m_d1[0] * rhs.m_d1[1] + lhs.m_d1[1] +
                                 rhs.m_d1[0] + lhs.m_val * rhs.m_d2[2]) +
                   lhs.m_d2[2] * rhs.m_val * rhs.m_val +
                   2 * lhs.m_val * rhs.m_d1[0] * rhs.m_d1[1]) /
                  (rhs.m_val * rhs.m_val * rhs.m_val);
  ...
  constexpr friend AAD22 exp(const AAD22 &func)
  {
    ...
    res.m_d2[0] =
        std::exp(func.m_val) * (func.m_d1[0] * func.m_d1[0] + func.m_d2[0]);
    res.m_d2[1] =
        std::exp(func.m_val) * (func.m_d1[1] * func.m_d1[1] + func.m_d2[1]);
    res.m_d2[2] =
        std::exp(func.m_val) * (func.m_d1[0] * func.m_d1[1] + func.m_d2[2]);
  ...
  constexpr friend AAD22 sin(const AAD22 &func)
  {
    ...
    res.m_d2[0] = std::cos(func.m_val) * func.m_d2[0] -
                  std::sin(func.m_val) * func.m_d1[0] * func.m_d1[0];
    res.m_d2[1] = std::cos(func.m_val) * func.m_d2[1] -
                  std::sin(func.m_val) * func.m_d1[1] * func.m_d1[1];
    res.m_d2[2] = std::cos(func.m_val) * func.m_d2[2] -
                  std::sin(func.m_val) * func.m_d1[0] * func.m_d1[1];
  ...
  constexpr friend AAD22 cos(const AAD22 &func)
  {
    ...
    res.m_d2[0] = -std::sin(func.m_val) * func.m_d2[0] -
                  std::cos(func.m_val) * func.m_d1[0] * func.m_d1[0];
    res.m_d2[1] = -std::sin(func.m_val) * func.m_d2[1] -
                  std::cos(func.m_val) * func.m_d1[1] * func.m_d1[1];
    res.m_d2[2] = -std::sin(func.m_val) * func.m_d2[2] -
                  std::cos(func.m_val) * func.m_d1[0] * func.m_d1[1];
  ...
  constexpr double get_x() { return m_d1[0]; }

  constexpr double get_y() { return m_d1[1]; }

  constexpr double get_xx() { return m_d2[0]; }

  constexpr double get_yy() { return m_d2[1]; }

  constexpr double get_xy() { return m_d2[2]; }
};

constexpr AAD22 F_AAD(AAD22 const &x, AAD22 const &y)
{
...
constexpr double Differentiator(const Collable &F, double x, double y)
{
  ...
  else if constexpr (M == DiffMethod::stencil5)
  {
    ...
    {
      return stencil5_func<WhichD::xy>(x, y, h_x, h_y);
    }
  }
  ...
  else if constexpr (M == DiffMethod::stencil5Extra)
  {
    ...
    {
      return (F(x + h_x / n, y + h_y / n) - F(x + h_x / n, y - h_y / n) -
              F(x - h_x / n, y + h_y / n) + F(x - h_x / n, y - h_y / n)) /
             (4 * h_x / n * h_y / n);
    }
  }
