...
public:
  ...
  // compute Mach number
  double compute_M(double v, double h)
  {
  ...
  std::tuple<double, double, double, double> compute_dt_u(double x, double v_x,
                                                          double y, double v_y)
  {
    double v = std::sqrt(v_x * v_x + v_y * v_y);
  ...
  RHS(double p_x, double p_v_x, double p_y, double p_v_y)
  {
    x = p_x;
    v_x = p_v_x;
    y = p_y;
  ...
  std::tuple<double, double, double, double> getRHS()
  {
  ...
  static constexpr double m = 106.0;            // kg - projectile mass
  static constexpr double d = 0.216;            // m - diameter
  static constexpr double v_0 = 1640;           // m/sec - initial velocity
  static constexpr double pi = 3.1415926535897; // pi constant
  static constexpr double S = pi * d * d / 4;   // projectile cross-section
};
