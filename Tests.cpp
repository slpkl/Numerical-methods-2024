...
void TestFunction(F x)
{
  ...
  if (TypeMetric <= ErrorLimit)
  {
    std::cout << "Test Passed!  "
              << " "
              << "Pow = " << x << ", " << std::setprecision(SET_PRECISION)
              << "Diff: " << TypeMetric << '\n';
    counter++;
    success_count++;
  }
  else
  {
    std::cout << "Test Failed!  "
              ...
              << "Pow = " << x << ", "
              << "Ideal: " << std::exp(x) << ", "
              << "Our: " << Exp(x) << ", " << std::setprecision(SET_PRECISION)
              << "Diff: " << TypeMetric << '\n';
    counter++;
    unsuccess_count++;
  }
...
int main()
{
  ...
  std::vector<double> double_arr = {
      -10.5, -5.0,  -4.66, -4.5,  -4.27, -3.66, -3.5, -3.2, -2.7, -2.56, -2.44,
      -2.1,  -1.97, -1.5,  -1.06, -0.7,  0.3,   0.4,  0.5,  0.6,  0.7,   1.1,
      2.44,  2.56,  2.7,   3.2,   3.5,   3.65,  4.27, 4.5,  4.66, 5.0,   10.5};
